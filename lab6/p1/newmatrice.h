  ...
  if (me == 0){
    for(m = 0; m < C->rows; m++){
      ...
      for(n = 0; n < C->cols; n++){
        for(i = 0; i < A->cols; i++){
          array_one[i] = A->arr[(m * A->cols)+i];
          array_two[i] = B->arr[(i * B->cols)+m];
        }
        destination = 1 + ((n + m*A->cols) % (world_size-1));
        printf("\nSending m: %d n: %d to destination: %d\n",m,n,destination);
        MPI_Send(array_one, array_size, MPI_INT, destination, 0, world);
        MPI_Send(array_two, array_size, MPI_INT, destination, 1, world);
        MPI_Send(&m, 1, MPI_INT, destination, 2, world);
        MPI_Send(&n, 1, MPI_INT, destination, 3, world);
      }
      test += 4;
    }

  } else {
    for(k = 0; k < (C->rows * C->cols)/(world_size-1); k++){
      printf("%d ", k);
      MPI_Recv(array_one, array_size, MPI_INT, 0, 0, world, &status);
      MPI_Recv(array_two, array_size, MPI_INT, 0, 1, world, &status);
      MPI_Recv(&m, 1, MPI_INT, 0, 2, world, &status);
      MPI_Recv(&n, 1, MPI_INT, 0, 3, world, &status);
...
void gauss_jordan(matrix* A, matrix* B){
  ...
  int *sendcounts = malloc(array_size * sizeof(int));
  int displs[world_size];
  ...
  float* L_array_A = malloc(array_size * sizeof(float));
  float* partial_array_A = malloc(array_size * sizeof(float));
  float* partial_array_Return = malloc(sendcounts[me] * sizeof(float));

  for(k = 0; k < A->rows; k++){
    for(j = 0; j < array_size; j++){
      for(i = 0; i < array_size; i++){
        if (me == 0){
          L_array_A[i] = (ACCESS(A,i,j)/(double)ACCESS(A,k,k));
        }
        L_array_A[i] *= ACCESS(A,k,i);
      ...
      if(k != j){
        ...
        /*
        for(i = 0; i < sendcounts[me]; i++){
          printf("%.3f - %.3f\n", partial_array_Return[i], L_array_A[i]);
          partial_array_Return[i] -= (double)L_array_A[i];
          //printf("new array return i: %.3f\n", partial_array_Return[i]);
        }
        */
        MPI_Gatherv(
          partial_array_Return,
      ...
      ACCESS(A,k,j) = partial_array_A[j];
    }
  }
}
